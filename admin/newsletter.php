...
if($_GET[cat]) //editing a newsletter
{
	...
	$selD = 'select * from days_to_send where category="'.$_GET[cat].'" order by dateField';
	$resD = mysql_query($selD, $conn) or die(mysql_error());
	
	while($d = mysql_fetch_assoc($resD))
	{
		$dateList .= $d[dateField].' <input type=submit name=del[] value=" Delete " 
		onclick="document.forms[0].deletethis.value=\''.$d[dateField].'\';"><br>';
	}
	
	$dateList = '<table cellspacing=0 class=thelist><tr><th>Release Dates</th></tr><tr><td>
	<form name=date>'.$dateList.'
	<input type=text name=dateField>
	<input type=submit name=dateAdd size=10 value="Enter Date">
