...
if( isset( $_POST['update'] ) && $_POST['update'] == 'yes' ) {
   ...
   // check nonce field
   $nonce = explode( ':', clean( $_POST['nonce'] ) );
   ...
   // check password
   if( !( check_member_password( $listing, clean( $_POST['email'] ),
      clean( $_POST['old_password'] ) ) ) )
   ...
   if( count( $messages ) == 0 ) {
      ...
      $member = get_member_info( $listing, clean( $_POST['email'] ) );

      // "new" email?
      if( $_POST['email_new'] == '' )
         $data['email_new'] = $member['email'];
      else
         $data['email_new'] = clean( $_POST['email_new'] );

      // new name?
      $data['name'] = ucwords( clean( $_POST['name'] ) );
      if( $data['name'] == '' )
      ...
      // new url
      if( $_POST['url'] == '' && !isset( $_POST['url_delete'] ) )
         $data['url'] = $member['url'];
      else
         $data['url'] = clean( $_POST['url'] );

      // show email address?
      $data['showemail'] = $member['showemail'];
      if( $_POST['showemail'] == 'show' )
         $data['showemail'] = 'show';
      else if( $_POST['showemail'] == 'hide' )
         $data['showemail'] = 'hide';
      else
      ...
      if( $info['additional'] != '' ) {
         ...
         foreach( $fields as $field ) {
            if( $field == '' ) continue;
            if( $_POST[$field] == '' && !isset( $_POST[$field . '_delete'] ) )
      ...
      // do actual update!
      $success = edit_member_info( $listing, clean( $_POST['email'] ), $data, 'hold' );
      if( $success ) {
...
?>
   ...
   <input type="hidden" name="nonce" value="<?php echo $rand ?>:<?php echo strtotime( date( 'r' ) ) ?>:<?php echo md5( $rand ) . substr( $rand, 2, 3 ) ?>" />
...
?>
         ...
         ?>_delete">
         <input type="checkbox" name="<?php echo $field ?>_delete" value="yes"
